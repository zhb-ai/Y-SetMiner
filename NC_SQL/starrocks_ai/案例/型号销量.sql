select
部门  ,业务员  ,客户编码  ,客户名称  ,大类,品牌,品类,小类,存货编码,存货名称,月份
,sum(价税合计) 价税合计  ,sum(流水) 流水,sum(数量) 数量,count(distinct vreceiptcode ) 订单量,count(distinct 商桥订单 ) 商桥订单量,count(distinct 日期 ) 频次
from (
select
ii.fstatus,
ii.csaleid as 主键,
...
join (select ib.csaleid,ib.nnumber,ib.nsummny,ch.invpinpai,
ch.height,ch.width,ch.length,ch.invname,ch.invcode,ib.blargessflag
,ch.pk_invbasdoc,ib.coriginalbillcode,csourcebillid,th.vreceiptcode,ib.noriginalcurmny
...
-- and ib.invname = '盈佳YJ Q2612A/303黑鼓易加粉-网络版-2K'
-- and nvl(m.vreceiptcode,th.vreceiptcode) = '589748859'
-- and  ii.dbilldate = to_char(sysdate,'yyyy-mm-dd')
and   ii.dbilldate >= '2024-01-01' 
and ib.invcode in ('CF0311') 
-- or (ii.dbilldate between  '2023-08-01' and '2024-02-01')
-- and cu.custname =  '新疆诚忠乐商贸有限公司'
-- and ii.dbilldate between  '2024-01-01' and '2025-01-31'
-- and ib.width = '兼容'
-- and ib.invname like '%CC388A%'
-- and cu.sf like '%北京%'
...
-- and  ii.dbilldate >= to_char(sysdate-91,'yyyy-mm-dd')
-- and ib.invname like '%定制%'
-- and yw.businame like '%电商%'
-- and cu.deptname <> '直营+微分销'
-- and ib.invpinpai like '%网络版%'
-- and ib.width  = '兼容'
-- and cu.custname in ('上海中飞办公设备有限公司')

) group by 部门  ,业务员  ,客户编码  ,客户名称  ,大类,品牌,品类,小类,存货编码,存货名称,月份

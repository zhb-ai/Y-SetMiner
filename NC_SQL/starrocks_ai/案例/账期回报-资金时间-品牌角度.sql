...
select 
 to_char(sysdate-1 ,'yyyy-mm-dd') as 当前日期
 ,nvl(d.大类,f.大类) as 大类
 ,nvl(d.品牌,f.品牌)as 品牌
 ,d.资金时间
 ,d.价税合计
 ...
 ,f.毛利率 
 from 
(select ma.大类,ma.品牌,sum(ma.资金时间) 资金时间 ,sum(ma.价税合计) 价税合计,sum(ma.资金时间)/28 as 拟平均应收,sum(ma.价税合计)/1.1 as  拟单月流水 
from (
select distinct 
...
full join 
(select
 ma.大类,ma.品牌,ma.部门,sum(ma.毛利)/sum(ma.流水) as 毛利率
 from (
...
where
  ...
  and '流水' <> 'null'
  and cg.width in ('兼容','原装','打印机')
  and (cu.n_deptname not like '%京东%'or cu.n_deptname not like '%苏宁%'or cu.deptname not like '%微分销%')
  ) ma 
where ma.流水 <> 0 and  ma.日期 between to_char(sysdate - 29 ,'yyyy-mm-dd') and to_char(sysdate -1 ,'yyyy-mm-dd') 
) ma group by ma.大类,ma.品牌,ma.部门) f on f.品牌 = d.品牌 

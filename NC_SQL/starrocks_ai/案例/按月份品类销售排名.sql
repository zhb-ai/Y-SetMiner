 select  d.存货编码,d.存货名称 , d.品牌,d.品类,dense_rank()OVER(PARTITION BY d.品牌,d.品类
          ORDER BY  sum(d.数量) desc ) AS 排名,sum(d.数量) 数量,to_char(d.日期,'MM') 月份 from (select ma.客户编码,ma.客户名称,ma.存货编码,ma.存货名称,ma.大类,ma.品类,ma.品牌,ma.小类,ma.数量,ma.是否赠品,ma.业务类型,ma.业务属性,ma.本币价税合计,ma.批发价税合计,ma.价税合计,ma.流水,ma.毛利,ma.成本,ma.市场部,ma.部门,ma.业务员,to_date(ma.日期,'yyyy-MM-dd') as 日期,ma.省,ma.市
  from (select 
  cu.custcode as 客户编码,
  cu.custname as 客户名称,
  cg.invcode as 存货编码,
  ...
  where  (ma.日期 between '2019-10-01' and  '2019-12-31'or ma.日期 between '2020-03-01' and  '2020-04-31')
    and ma.大类 = '兼容' ) d group by  d.存货编码,d.存货名称,d.品牌,d.品类,to_char(d.日期,'MM')

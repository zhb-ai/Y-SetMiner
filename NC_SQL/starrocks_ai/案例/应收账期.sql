 select  cu.部门,cu.业务员,cu.客户名称,
  ...
  sum(case when cu.cse =  '四到十天' then cu.余额 else null end) 四到十天,
    ...
    from (
select cu.部门 ,cu.业务员,cu.客户名称 ,cu.czq,
...
select 
 ...
 group by  ddh, billdate,czq, djdl,tyzq, zy, hbbm,businame order by 1) cu  --where  cu.客户名称 ='济南颖熙信息技术有限公司' 
  group by   cu.部门 ,cu.业务员,cu.客户名称 ,cu.czq)cu group by  cu.部门,cu.业务员,cu.客户名称

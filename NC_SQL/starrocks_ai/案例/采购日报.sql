select ma.日期,ma.打印品牌,ma.大类,ma.部门划分,ma.客户名称,ma.业务员,sum(ma.流水) as 流水,sum(ma.毛利) as 毛利 ,case when ma.打印品牌 in ('爱普生打印机','爱普生','利盟') then '白晓野'
when ma.打印品牌 in ('惠普黑鼓','三星','OKI') then '巩林波'
when ma.打印品牌 in ('惠普打印机（渠道）','其它品牌打印机','佳能墨盒','佳能硒鼓') then '何路超'
  when ma.打印品牌 in ('施乐','兄弟','富士通','联想') then '侯志祥'
      when ma.打印品牌 in ('奔图打印机','惠普墨盒','奔图','其它打印类') then '李晶伟'
         when ma.打印品牌 in ('佳能复印类','施乐复印类','三星复印类','柯尼卡美能达','东芝','夏普','理光','京瓷','其他复印类') then '王红曼'
          when ma.打印品牌 in ('惠普打印机（行业）') then '姚辉铭'
          when ma.打印品牌 in ('佳能打印机') then '张京'
            when ma.打印品牌 in ('惠普彩鼓') then '张石林'
               when ma.打印品牌 in ('扬帆耐立','盈佳','智通') then '张曦'
...
where
  ...
  and js.dr = 0
  -- '流水' <> 'null' 
  and (case when js.dapprovedate is null then js.dmakedate else js.dapprovedate end ) >=  to_char(sysdate - 1,'yyyy-mm-dd') 
  ) ma where ma.大类 in ('打印机','原装','兼容')) ma
   group by ma.打印品牌,ma.大类,ma.部门划分,ma.客户名称,ma.业务员,ma.日期

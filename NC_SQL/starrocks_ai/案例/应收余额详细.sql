select a.*,case when 市场部='直营+微分销' then '电商业务' else '渠道业务'end 市场部划分  from (
select  cu.市场部, cu.部门,cu.业务员,cu.客户主键,cu.客户名称,
sum(cu.余额) as 余额
,sum(预收)as 预收
,sum(cu.余额) + sum(预收) as 实际应收
,sum(超期应收) as 超期应收,
min(min_date) min_date,
max(max_date) max_date 
from (
select cu.部门 ,cu.业务员,cu.客户名称,cu.客户主键 ,cu.市场部,cu.czq,
case when cu.czq>0 then '账期外'else '账期内'end 是否超期, 
case when cu.czq>0 and cu.czq<=1 then '一到三天'
when  cu.czq>1 and cu.czq<=10 then '四到十天'
...
group by  ddh, billdate,czq, djdl,tyzq, zy, hbbm,businame,psnname order by 1) cu -- where    cu.业务员 = '孙希宝'-- where  cu.客户名称 <>'智通仁和办公拼购专营店' 
group by  cu.市场部,cu.部门 ,cu.业务员,cu.客户名称,cu.客户主键 ,cu.czq)cu 
group by  cu.部门,cu.业务员,cu.客户名称,cu.客户主键,cu.市场部) a where 实际应收> 0

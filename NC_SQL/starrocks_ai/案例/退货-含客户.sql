...
on ic_general_h.ccustomerid = cu.pk_cumandoc
left join nc5x.bd_psndoc  us
on us.pk_psndoc= ic_general_h.cbizid
where  daccountdate between '2023-01-01' and '2023-12-31'and ic_general_b.dr = 0 
and bd_billtype.billtypename like '%销售出库%'  
and length(c.invcode)> 4
and bd_stordoc.storname like '%残%'
and ic_general_b.noutnum < 0 
group by 

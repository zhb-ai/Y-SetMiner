...
select ma.客户名称,ma.存货名称,ma.存货编码,ma.品类 ,ma.品牌,
sum(case when ma.日期 >= to_char(sysdate - 8,'yyyy-mm-dd')then  ma.流水 else 0 end ) as 七天流水,
sum(case when ma.日期 >= to_char(sysdate - 31,'yyyy-mm-dd') then ma.流水 else 0  end ) as 三十天流水,
sum(case when ma.日期 >= to_char(sysdate - 61,'yyyy-mm-dd') then  ma.流水 else 0  end ) as 六十天流水,
sum(case when ma.日期 >= to_char(sysdate - 8,'yyyy-mm-dd')then  ma.毛利 else 0 end ) as 七天毛利,
sum(case when ma.日期 >= to_char(sysdate - 31,'yyyy-mm-dd') then ma.毛利 else 0  end ) as 三十天毛利,
sum(case when ma.日期 >= to_char(sysdate - 61,'yyyy-mm-dd') then  ma.毛利 else 0  end ) as 六十天毛利,
sum(case when ma.日期 >= to_char(sysdate - 8,'yyyy-mm-dd')then  ma.数量 else 0 end ) as 七天数量,
sum(case when ma.日期 >= to_char(sysdate - 31,'yyyy-mm-dd') then ma.数量 else 0  end ) as 三十天数量,
sum(case when ma.日期 >= to_char(sysdate - 61,'yyyy-mm-dd') then  ma.数量 else 0  end ) as 六十天数量,
sum(ma.数量) as 数量,sum(ma.毛利) as 毛利,count(distinct ma.客户名称) as 客户数
 from  (
...
where 
  ...
  where ma.日期 between '2020-11-01'and '2021-01-31'
  and  ma.大类   in ('兼容') and ma.品牌 = '盈佳' and ma.存货名称 ='盈佳YJ FX-P355黑粉高容(带芯片)'
-- and ma.客户编码 in ('1101b179','1101b180','1101b181','13581995378')
 ) ma  group by ma.客户名称,ma.存货名称,ma.存货编码,ma.品类,ma.品牌

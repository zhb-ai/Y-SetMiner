select  出库单号,库房,大类,品牌,品类,存货名称 ,sum(数量)数量 from (
select cu.deptname  as 市场部,
  ...
  join (select ibb.csaleid,w.invname,w.width,ibb.nnumber,ibb.noriginalcurnetprice,w.unitvolume,w.unitweight ,ibb.nsummny,w.length,w.invpinpai
          from nc5x.so_saleinvoice_b ibb
          join nc5x.bd_invbasdoc w
            on ibb.cinvbasdocid = w.pk_invbasdoc)ibb
    on ibb.csaleid = ii.csaleid
 where ii.dbilldate >= '2022-07-01'
   and ii.dbilldate <= '2022-07-31'
   
   -- and  cu.deptname not in ('直营+微分销','大客户部','项目部')
   ) 
group by 出库单号,库房,大类,品牌,品类,存货名称

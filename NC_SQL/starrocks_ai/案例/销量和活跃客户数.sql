select c.psnname,c.custname,c.区域,d.* from (select c.*,case when c.psnname ='但波' then '贵州'
    ...
    from nc5x.v_zlw_cust  c where c.deptname ='区域销售部'and  c.linkman1 is not null)c  left join 

(select ma.客户名称,ma.存货编码,ma.存货名称,ma.单据号,ma.大类,ma.品类,ma.品牌,ma.小类,ma.数量,ma.是否赠品,ma.市场部,ma.部门,ma.业务员,ma.业务员1,ma.业务员2,to_char(to_date(ma.日期,'yyyy-MM-dd'),'yyyy-MM-dd') as 日期,ma.流水,ma.毛利,ma.省,ma.市,case when substr(ma.日期,6,2)='01' then '1月'
...
--SUBSTR(ma.日期,1,10) between  '2020-06-01'and '2020-06-27'
 ma.部门 ='区域销售部'
)d on d.客户名称=c.custname    where  d.业务员1=d.业务员2 and 年份 = 2020

select  to_char(sysdate ,'yyyy-mm-dd') as 当前日期,nvl(d.大类,f.大类) as 大类,nvl(d.品牌,f.品牌)as 品牌,d.资金时间,d.价税合计,d.拟平均应收,d.拟单月流水,f.毛利率 from 
(select ma.大类,ma.品牌,sum(case when ma.部门 like '%淘宝%' or ma.部门 like '%天猫%'or ma.部门 like '%苏宁%'or ma.部门 like '%京东%'or ma.部门 like '%京喜%'or ma.部门 like '%拼多多%'
or ma.部门 like '%渠道256%' then 0 else  ma.资金时间 end ) 资金时间,
sum(case when ma.部门 like '%淘宝%' or ma.部门 like '%天猫%'or ma.部门 like '%苏宁%'or ma.部门 like '%京东%'or ma.部门 like '%京喜%'or ma.部门 like '%拼多多%'
or ma.部门 like '%渠道256%' then 0 else ma.价税合计 end ) 价税合计,
sum(case when ma.部门 like '%淘宝%' or ma.部门 like '%天猫%'or ma.部门 like '%苏宁%'or ma.部门 like '%京东%'or ma.部门 like '%京喜%'or ma.部门 like '%拼多多%'
or ma.部门 like '%渠道256%' then 0 else ma.资金时间 end )/28 as 拟平均应收,
sum(case when ma.部门 like '%淘宝%' or ma.部门 like '%天猫%'or ma.部门 like '%苏宁%'or ma.部门 like '%京东%'or ma.部门 like '%京喜%'or ma.部门 like '%拼多多%'
or ma.部门 like '%渠道256%' then 0 else ma.价税合计 end )/1.1 as  拟单月流水 
from (
select   distinct 
...
) ma  group by ma.大类,ma.品牌)d

full join 
(select 
 ma.大类,ma.品牌,sum(ma.毛利)/sum(ma.流水) as 毛利率
 from (
...
where ma.流水 <> 0 and  ma.日期 between to_char(sysdate -29 ,'yyyy-mm-dd') and to_char(sysdate -1 ,'yyyy-mm-dd')) ma group by ma.大类,ma.品牌) f on f.品牌=d.品牌 

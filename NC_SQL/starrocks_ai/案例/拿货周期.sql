select d1.市场部,d1.部门,d1.业务员,d1.客户名称,d1.大类,d1.品牌,d1.品类,d1.小类,d1.存货名称,d1.最新采购日期,d2.后一次数量,d1.客户日销  ,dense_rank()OVER(PARTITION BY d1.市场部,d1.部门,d1.业务员,d1.大类,d1.品牌,d1.品类,d1.小类,d1.客户名称,d1.存货名称 ORDER BY d1.最新采购日期   ) AS 排名
 from (select d1.市场部,d1.部门,d1.业务员,d1.客户名称,d1.大类,d1.品牌,d1.品类,d1.小类,d1.存货名称,max(d1.日期) 最新采购日期 ,sum(d1.客户日销)/count(d1.客户日销) 客户日销
 from  (select d1.市场部,d1.部门,d1.业务员,d1.客户名称,d1.大类,d1.品牌,d1.品类,d1.小类,d1.存货名称,d1.日期 ,d1.销量  ,(d1.日期-d2.日期) 时间间隔,(d2.销量/(d1.日期-d2.日期)) 客户日销   from  
--------------------
(select * from                                                           
(select d.市场部,d.部门,d.业务员,d.大类,d.品牌,d.品类,d.小类,d.客户名称,d.存货名称,d.日期,sum(d.数量) 销量,sum(d.流水) 流水,max(d.数量) 最大值 ,count(d.客户名称) 拿货频次 ,dense_rank()OVER(PARTITION BY d.市场部,d.部门,d.业务员,d.大类,d.品牌,d.品类,d.小类,d.客户名称,d.存货名称 ORDER BY d.日期  Asc ) AS 排名 from 
...
and to_char(d1.日期,'yyyy-mm-dd')>= '2020-01-01') d1 group by d1.市场部,d1.部门,d1.业务员,d1.客户名称,d1.大类,d1.品牌,d1.品类,d1.小类,d1.存货名称 )d1 inner join
------------------------------------------------------------------------------------------------
 ...
 (select d1.市场部,d1.部门,d1.业务员,d1.客户名称,d1.大类,d1.品牌,d1.品类,d1.小类,d1.存货名称,d1.日期 后一次日期,d1.销量 后一次数量 ,(d1.日期-d2.日期) 时间间隔,(d2.销量/(d1.日期-d2.日期)) 客户日销   from  
--------------------
(select * from                                                           
(select d.市场部,d.部门,d.业务员,d.大类,d.品牌,d.品类,d.小类,d.客户名称,d.存货名称,d.日期,sum(d.数量) 销量,sum(d.流水) 流水,max(d.数量) 最大值 ,count(d.客户名称) 拿货频次 ,dense_rank()OVER(PARTITION BY d.市场部,d.部门,d.业务员,d.大类,d.品牌,d.品类,d.小类,d.客户名称,d.存货名称 ORDER BY d.日期  Asc ) AS 排名 from 
(select ma.客户名称,ma.存货编码,ma.存货名称,ma.单据号,ma.大类,ma.品类,ma.品牌,ma.小类,ma.数量,ma.是否赠品,ma.市场部,ma.部门,ma.业务员,to_date(ma.日期,'yyyy-MM-dd') as 日期,ma.流水,ma.毛利,ma.省,ma.市
...
and d1.排名=d2.排名+1
--and d1.客户名称= '阿巴嘎旗千凯电脑耗材经销部'
and to_char(d1.日期,'yyyy-mm-dd')>= '2020-01-01') d2  on d1.业务员=d2.业务员 where  d1.客户名称=d2.客户名称 and d1.存货名称=d2.存货名称 and d1.最新采购日期=d2.后一次日期

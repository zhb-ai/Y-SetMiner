select  cu.部门,cu.业务员,cu.客户主键,cu.客户名称,cu.市场部,
...
select 
  ...
  from (select ddh,
         ...
         where fb.djdl in ('ys', 'sk')
          ...
          and nvl(fb.bbye, 0) <> 0
           and fb.dr = 0
         and fb.isverifyfinished = 'N' and fb.djbh  <> 'SK1403070006'
 ...
 group by  ddh, billdate,czq, djdl,tyzq, zy, hbbm,businame,psnname order by 1) cu -- where    cu.业务员 = '孙希宝'-- where  cu.客户名称 <>'智通仁和办公拼购专营店' 
  group by   cu.市场部,cu.部门 ,cu.业务员,cu.客户名称,cu.客户主键 ,cu.czq)cu group by  cu.部门,cu.业务员,cu.客户名称,cu.客户主键,cu.市场部

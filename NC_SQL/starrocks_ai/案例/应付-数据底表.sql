...
(CASE djdl WHEN 'yf' then '应付' when 'fk' then '付款' else '其他' END) AS 单据大类,
ddh as  订单号,
...
and nvl(fb.bbye, 0) <> 0
and fb.dr = 0 
and fb.isverifyfinished = 'N'  and custname <> '北京东方超技科技发展有限公司'

) rf
  where 1=1-- and ddh = 'FDYT22063000001'
 group by  billdate,czq, djdl,tyzq, zy, hbbm,businame ,ddh ,invpinpai,height,width,length
